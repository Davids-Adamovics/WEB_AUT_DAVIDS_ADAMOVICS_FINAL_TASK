export class AutomationPractice{
    ...
    static get uploadImg(){
        return cy.get('input[type=file][id="uploadPicture"]').selectFile('cypress/files/img.png')
    }
    ...
    static get selectCity(){
     ...
     static get tbodyRowValid(){
        cy.get('tbody').within(() => {
            cy.contains('tr', 'Student Name').should('contain', 'Davids Adamovics');
            cy.contains('tr', 'Student Email').should('contain', 'testemail@gmail.com');
            cy.contains('tr', 'Gender').should('contain', 'Male');
            cy.contains('tr', 'Mobile').should('contain', '6766564659');
